...
void *filler(void *num)
{
    ...
    int n = *(int*) num;
    /* for every pixel */
    for( int px=n; px<width; px+=nthreads )
    {
	    ...
	    array[px][py][0] = scaled_color[0];
	    array[px][py][1] = scaled_color[1];
	    array[px][py][2] = scaled_color[2];
	}
    }
    return NULL;
}
...
main( int argc, char **argv )
{
    ...
    printf( "# Rendering scene with %d spheres and %d lights\n",
      ...
      {
          thrN[i] = i;
	  rtn = pthread_create(&thr[i], NULL, filler, &thrN[i]);
	  if (rtn)
	  {
